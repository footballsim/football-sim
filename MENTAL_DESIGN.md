...
## 4b. 体力消費（疲労）層 — DEV_NOTES①（2026-07-03 追加・ユーザー指示）

「デュエル参加ごと」ではなく**時間の経過**で全出場選手の能力が低下する。

```
低下率 = MAX_DROP(0.30) × (100-スタミナ)/100 × (出場経過)²   → 係数 floor 0.80
```

- **スタミナ100→90分間無低下（ユーザー仕様）／50→90分で-15%／85(主力平均)→-4.5%**。当初案「50→-50(絶対値)」は実サッカー（終盤の高強度ラン低下20-40%・技術実効値低下は数%〜15%）とk=2.7の増幅（95:5化）に照らし過大のため縮小＝ユーザー合意。
- **2乗カーブ**＝前半ほぼ無傷（45分で最大の25%）→60分以降に効く＝実際の交代適期と一致。延長はPROG_CAP(1.2)まで深化＝floor到達。
- **経過は選手ごとの出場チャンス数**（`fatigueOnChanceEnd`で加算・`_pitchChances`）＝**途中出場はフレッシュ**→交代に実効果。**GK対象外**（走行負荷小＋内部スタミナ50固定）。
- 効果配線はメンタルと同じ `getActionParam` の係数f（`fatigueParamFactor`・mental.js 4b）。既存 `p.fatigue`（デュエル参加カウント・AI交代目安）とは独立。キルスイッチ `FATIGUE_ENABLED=false`。
- dbgの「開始値」逆算と総合力表示は**メンタル×疲労の合成係数 `labParamFactor`** を使用（メンタルだけで割ると開始値に疲労分が混入）。
- KPI(N=9000): 平均合計得点 3.17→3.05（実W杯目安2.7へ近づく）。日本vsブラジル勝敗が許容差超過（スタミナ構成差）＝意図的変更としてbaseline更新（ユーザー承認後）。
