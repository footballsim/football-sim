...
- **Codex 側の実行許可**: `.codex/rules/football-sim.rules` — プロジェクトがtrustedな場合、workspace外の固定gateway `~/.codex/bin/football-sim-delegated-git.mjs`だけを都度確認なしで許可する。直接git書込はprompt、`git add ./-A`、reset/clean、`main` push、`npx`/`npm exec`/wrangler直叩きは機械的に禁止する。gatewayとRulesの変更自体は例外ゲート。OpenAI Docsの仕様上、新規rulesはCodex再起動後に確実に読み込まれる。
