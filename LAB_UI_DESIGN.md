...
| `press_wall` | 記者会見のバックパネル（試合後 見出しコマ） | 1280×720 | 濃紺＋ロゴ格子 |
...
> 発注時の注意: 全て **16:9 か tileable**。監督室系は**中央に情報を置くのでコントラストを落とし気味に**。
...
最後に「監督室へ戻る」→ `showScreen('home')` + `_renderHub(false)`。
...
- 本番 `docs/` への反映 — lab のみ（メモリ: 本番凍結）。
- 記者会見の対話・ボードとの交渉など**新規ゲームシステム**の追加（UIの器だけ先に作る）。
