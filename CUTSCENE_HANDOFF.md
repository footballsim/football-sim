...
| ショートパス | **統合トレース等は承認済み。ユーザー選択F3（4枚中左から3番目）の視線修正版を最終確認待ち** | 動画は [rnqIgNSQ-is](https://www.youtube.com/watch?v=rnqIgNSQ-is)、時刻は17.0/18.0/19.0/20.0/21.0/22.2秒。脚・腕・統合版・各コマ・姿勢中立スタイルボード・右向き刈上げ頭部・5.6頭身体型は `design/SHORTPASS_APPROVAL.json` にSHA-256固定。2026-08-25、クロス採用時の局所反復へ戻し、ユーザーは提示4枚のうち左から3番目を最良と選択。追加要望の「足元のボール接触位置を見る」は、選択画像の三角形の目と全身を固定し、白目内部23pxだけで瞳を右下へ修正したv3が独立QAをPASS。旧手動v7、誤命名v11、生成v11〜v18、座標直描画、ローカルv20、Replicate試行は不採用・再利用禁止。 | まずF3視線修正版v3だけをユーザーが最終判定する。採用までは本編統合、build、push、deploy、残り5コマ生成を禁止。却下時は選択画像の合格領域を固定し、目の指摘部位だけを再編集する。採用後もF1/F2/F4/F5/F6を1コマずつ同じ局所反復で作り、全体再生成や有料外部経路へ戻らない。 |
...
5. ユーザー選択F3（4枚中左から3番目）の視線修正版v3は独立QAを通過済みで最終目視待ち。却下時は同画像の合格領域を固定し、目の指摘部位だけを局所修正する。明示採用まで残り5コマを生成しない。
