...
| ショートパス（5コマ） | **ユーザー決定でF1を削除。F2〜F6の5コマ構成。F2/F3採用済み。F4は面トレースv18承認済みだが、画像候補は未採用。F5/F6は未生成** | 動画は [rnqIgNSQ-is](https://www.youtube.com/watch?v=rnqIgNSQ-is)。制作中の元番号と時刻はF2=18.0、F3=19.0、F4=20.0、F5=21.0、F6=22.2秒。F2/F3正典はSHA固定。F4のsemantic/review/overlay/exact-controlはtracked authority-packとして固定するが、生成候補のF4本体とは別物であり採用扱いにしない。 | 2026-08-27指示により全カットシーン向け決定論パイプラインPhase 1を実装する。許可はF2追跡固定、manifest、authority verification、CT4Dのgeometry-onlyによるF2/F3再現準備。metrics／候補評価はPhase 2較正完了まで禁止。画像生成、F5/F6、採用済みF4本体の格納・昇格、本編統合、build、push、deployは禁止。 |
...
3. Phase 1では画像生成・候補入力・metrics判定を実行しない。`cutscene_production_gate.py`はauthority-packの存在、SHA、寸法、mode、tracked状態だけを検証し、候補認定はPhase 2の較正済み評価器までロックする。
4. 旧 `shortpass_approval_gate.py --mode generation` 経路は失効済みで、Phase 2解除までは実行禁止。F2/F3の再現準備はCT4Dのgeometry-only工程として別途設計・較正する。
5. F3は `img/cutscenes/manga_shortpass6/frame_03.png` をユーザー採用済み正典として使う。白目過多で却下された直前v3や他候補へ戻さない。
6. F1は2026-08-26のユーザー決定でシーンから削除済み。F2はユーザー採用済み正典として固定。F4は衣服面トレースv18まで承認済みだが、画像候補は未採用。v12はユーザー却下、v4〜v13は再利用禁止、v14は構造のみ、v15は失敗方式、v16/v17は内部編集ベース、v18は独立QA FAIL、v19は内部却下。F4のユーザー判断前にF5/F6を生成しない。
7. 制作中は承認トレースとの対応を失わないようF2〜F6の元番号を維持する。全5枚採用後の最終統合時だけ `F2→frame_01`, `F3→frame_02`, `F4→frame_03`, `F5→frame_04`, `F6→frame_05` に写像する。
