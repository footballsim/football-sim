...
| ショートパス | **統合トレース等は承認済み。クロス方式で作ったF3候補v4をユーザー確認待ち** | 動画は [rnqIgNSQ-is](https://www.youtube.com/watch?v=rnqIgNSQ-is)、時刻は17.0/18.0/19.0/20.0/21.0/22.2秒。脚・腕・統合版・各コマ・姿勢中立スタイルボード・右向き刈上げ頭部・5.6頭身体型は `design/SHORTPASS_APPROVAL.json` にSHA-256固定。2026-08-25、未実証の有料ハード姿勢経路を優先した判断を撤回し、クロス採用時に成功した「1コマ生成→合格領域を凍結→問題部位を1つずつ局所修正→比較」へ戻した。F3は蹴り脚、右肘、上体軸だけを順に修正したv4が独立QAで、方向、脚左右、両腕、肩/胴、5.6頭身、刈上げ、画風、全身クロップをPASS。外周の近白背景だけを人物再生成なしで純白化した。旧手動v7、誤命名v11、生成v11〜v18、座標直描画、ローカルv20、Replicate試行は不採用・再利用禁止。 | まずF3候補v4だけをユーザーが目視判定する。採用までは本編統合、build、push、deploy、残り5コマ生成を禁止。却下時はv4の合格領域を固定し、指摘された1部位だけを再編集する。採用後もF1/F2/F4/F5/F6を1コマずつ同じ局所反復で作り、全体再生成や有料外部経路へ戻らない。 |
...
5. F3候補v4は独立QAを通過済みでユーザー目視待ち。ユーザーが却下した場合はv4の合格領域を固定し、指摘部位だけを局所修正する。明示採用まで残り5コマを生成しない。
6. F3採用後にのみF1・F2・F4・F5・F6を1コマずつ生成する。各コマで復元身長`5.40–5.80H`、頭幅`0.97–1.07H`、肩幅`1.32–1.48H`、頭高のフレーム間差3%以内（正規化後原則`±2px`）、向き、左右、遮蔽、白背景、刈上げ、CT4画風を検査する。全合格後だけ上段=承認トレース、下段=生成画像の比較を提示し、採用回答まで本編へ統合しない。
