...
| 通常シュート | **4コマ版ユーザー採用済み・本編配線済み（デプロイ工程）** | `shot4new`＝`_renderAdoptedShotScene`／`manga_shot_adopted` は2026-08-30にユーザーが採用版と明示訂正した。通常シュートの蹴る1ビートへ接続し、枠外／GK／ゴールの結果分岐は維持。`shot4cinematic`＝追加シネマチック4拍は未採用・比較専用のまま。 | QA、build、push、kantoku-lab反映を完了する。 |
...
| ショートパス（5コマ） | **F2〜F6全コマをユーザー採用済み・canonical/Lab反映済み・本編配線済み（デプロイ工程）** | Phase 1/2A時点の「F4未採用・F5/F6未生成」は、その後のユーザー採用で失効。F6→`frame_05.png` は承認済み足首角度v5（SHA `d2956ada...`）。画面左側=左軸足、画面右側=右蹴り足。外周白残り除去、F2シャツ明度、パンツ色、選手・ボール18px左寄せ、24px右移動、F5接触からのボール飛行を維持し、通常ショートパスだけ本編接続。ワンツー／失敗経路は維持。 | QA、build、push、kantoku-lab反映を完了する。 |
