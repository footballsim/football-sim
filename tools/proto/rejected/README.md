...
## foul_atk_wavy_lip_dist4_rejected_20260717.png (2026-07-17)
上唇の黒除去(CAVITY_R=4 距離塗り分け)版・asset-qa FAIL。却下理由:
- 上唇の黒帯削減という目的は達成（実寸34%で 5px→2-3px・目標3px）が、幾何距離の円が
  「絵に描かれた唇線」を無視して横切り、口インクを1塊210px → 156+10+3+2px に破断
  - x588-593,y117-118(10px)=上唇ラインが本体から分離し宙に浮く（y119が全て肌置換で切断）
  - x594-596,y120(3px) / x598-599,y122(2px)=肌上の孤立黒点＝頬のゴミ。34%でも茶色のシミとして残存
- 根本原因: 距離D=2..6のどの値でも「距離円 vs 描線」のズレは消えない。距離ではなく
  **インクの連結性**（塗り替え後も口インクが単一連結成分・孤立断片なし）を制約にすべき
- 私のゲート（明→純黒0px / diff bbox⊂box）は「元から黒い画素が取り残される」欠陥を
  原理的に検出できない＝後追い検査のアンチパターン。連結成分数と最小断片サイズをゲートに追加すること
- なお「肌ベタ塗りが平坦に見える」という私の懸念は空振り（充填色253,171,76 vs 隣接肌 mean242,171,83 で妥当）
